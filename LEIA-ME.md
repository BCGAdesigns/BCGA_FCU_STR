...
A **BCGA FCU** é uma Fire Control Unit digital para replicas airsoft HPA (High Pressure Air), baseada no ESP32-C3 SuperMini. Ela controla o(s) solenoide(s) que substituem o gatilho mecânico do engine HPA e permite ajustar cada fase do ciclo de disparo pelo browser via WiFi — sem precisar de app.
...
2. **4 timings independentes (DN/DR/DP/DB).** Cada fase do ciclo D8PA tem seu próprio parâmetro. Alimentação (DN), vedação (DR), FPS (DP) e debounce pós-tiro (DB) ajustam independente, sem trade-offs. FCUs comerciais chegam no máximo a 3 timings expostos (REV3 em modo dual-solenoid: dP/dN/dr) ou dwell único em single-solenoid — nenhuma expõe parâmetro dedicado para debounce pós-tiro.
...
10. **Troca S8PA/D8PA por slot via software.** Cada um dos 3 slots armazena independentemente o tipo de engine — alterne o slot entre S8PA e D8PA direto pelo painel web, sem mexer em hardware. FCUs comerciais que cobrem as duas arquiteturas (TITAN II com PULSAR S/D, Gorilla) fazem isso via cabos dedicados distintos por engine, não via chaveamento por slot no software.
...
- **Sem corte automático de bateria.** O firmware entra em modo lockout (firing bloqueado, beep contínuo) quando a bateria atinge nível crítico na variante PRO, mas você precisa desconectar fisicamente a bateria para preservar a carga. A variante STR não tem leitura de bateria — use com cuidado em packs 2S/3S sem proteção externa.
- **Bateria drena quando conectada.** Os reguladores de tensão da placa PRO consomem ~12-18mA continuamente mesmo com o ESP32 dormindo. Um pack de 1000mAh descarrega totalmente em 2-3 dias se ficar conectado. Sempre desconecte a bateria entre sessões.
...
| Deep sleep | 60 min de inatividade → ESP32 <10 µA, placa total ~12-18 mA (reguladores) |
| Alarme de inatividade | 60 min ocioso → 6 ciclos de 5min beep + 1h sleep, depois sleep permanente |
| Gestão de bateria (PRO) | Modo lockout na tensão crítica (sem corte automático — desconexão física obrigatória) |
| Licença | GPL v3 |
...
├── DaugtherBoard_BCGA/       # Daughterboard A — engine BCGA (S8PA ou D8PA)
...
5. Calibre o gatilho Hall e o seletor pela interface web (seções 7 e 8 do manual).
