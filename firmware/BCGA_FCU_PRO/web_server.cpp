...
void handleSave() {
  ...
  if (body.containsKey("solenoids"))   c.solenoidCount = (uint8_t)constrain((int)body["solenoids"], 1, 2);
...
void handleTest() {
  ...
  if (body.containsKey("buzz")) {
    ...
    uint8_t pin = 0;
    // "mos: 1" pulses the logical POPPET (SOL1) pin; "mos: 2" the NOZZLE (SOL2).
    // Both respect mosfetSwap, so users see the physical wiring effect.
    if (which == 1)      pin = pinPoppet(c);
    else if (which == 2) pin = pinNozzle(c);
...
void handleNoiseCal() {
  ...
  // Returns observed min/max ADC values. Blocks for ~75ms.
  if (firingActive()) { sendErr(409, "firing active"); return; }
  if (!server.hasArg("plain")) { sendErr(400, "no body"); return; }
