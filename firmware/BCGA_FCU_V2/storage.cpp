...
void storageDefaultSlot(uint8_t idx, SlotConfig& out) {
...
#ifdef BCGA_PRO
  out.solenoidCount = DEFAULT_SOLENOIDS;   // D8PA by default on Pro
#else
  out.solenoidCount = 1;                   // Starter has only 1 MOSFET → S8PA
#endif
  ...
  out.mosfetSwap    = 0;
  out.invertTrig    = 0;                   // label is just "Inverter gatilho" — off by default
  out.silentMode    = 0;
}
