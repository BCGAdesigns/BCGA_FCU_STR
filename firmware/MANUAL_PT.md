...
2. [Início rápido — chegar no FPS alvo primeiro](#2-inicio-rapido--chegar-no-fps-alvo-primeiro)
3. [Os 4 timings — guia de tuning](#3-os-4-timings--guia-de-tuning)
...
14. [Deep sleep, alarme de inatividade e modo debug](#14-deep-sleep-alarme-de-inatividade-e-modo-debug)
...
A FCU controla **1 ou 2 solenoides** que substituem o gatilho mecânico de um engine HPA. Em cada tiro, ela envia pulsos eletricamente calibrados para os solenoides, na ordem correta, com as esperas corretas entre eles. Tuning = ajustar esses pulsos para o teu sistema específico (pressão, bucking, BB, peso).

- **S8PA** (1 solenoide): PolarStar JACK / F1, Wolverine INFERNO, GATE PULSAR S, ou qualquer outro engine HPA de 1 solenoide. Só o poppet é controlado.
- **D8PA** (2 solenoides): PolarStar F2 / Fusion Engine, GATE PULSAR D, ou qualquer outro engine HPA de 2 solenoides. Nozzle + poppet controlados separadamente.

---

## 2. Início rápido — chegar no FPS alvo primeiro

Primeira vez na BCGA FCU? Antes de tudo, acerte o FPS alvo. O resto do tuning só faz sentido com o chrono onde você quer. Os 4 timings (DN/DR/DP/DB) estão explicados na seção 3 — nessa primeira passada você só mexe no **DP** (o slider do poppet dwell) e no regulador.
...
Siga nesta ordem — cada passo depende do anterior estar estável. Assume que você já atingiu o FPS alvo pela seção 2.
...
- **S8PA** — só o poppet é pulsado. Ciclo: `DP → DR → repete`. Use com PolarStar JACK / F1, Wolverine INFERNO, GATE PULSAR S, ou qualquer engine HPA de 1 solenoide. Os campos DN, DB, swap MOS e o botão de teste do SOL 2 somem da interface.
...
## 14. Deep sleep, alarme de inatividade e modo debug

### Alarme de inatividade (NOVO)

Após **60 minutos sem qualquer atividade do usuário** (puxada de gatilho, botão WiFi, mudança de seletor ou interação no painel web), a FCU entra numa sequência de alarme desenhada para chamar sua atenção antes de dormir:

- **6 ciclos de alarme**, cada um composto por:
  - 5 minutos de beeps curtos a cada 30 segundos
  - Depois 1 hora de deep sleep
  - Depois acorda e inicia o próximo ciclo
- Ao completar os 6 ciclos (~6 horas no total), a FCU entra em **deep sleep permanente**
- **Qualquer atividade reseta o contador** e volta à operação normal

Isso protege contra esquecer a FCU ligada, mas **não preserva a bateria indefinidamente** — veja abaixo.

### Comportamento do deep sleep

Quando a FCU entra em deep sleep:
- O ESP32-C3 consome **<10 µA**
- Os reguladores de tensão da placa PRO (MP2315 buck, MT3608 boost, AMS1117 LDO) **continuam consumindo ~12-18 mA** enquanto a bateria estiver conectada
- Consumo total da placa em deep sleep: ~12-18 mA (só os reguladores)

Uma bateria 1000 mAh descarrega totalmente em **2-3 dias** se ficar plugada, mesmo com o ESP32 dormindo.

> ⚠️ **A única forma de realmente preservar a bateria entre sessões é desconectá-la fisicamente.**

### Despertar (wake-up)

Após deep sleep, a próxima puxada de gatilho **acorda o MCU via reboot completo** — a segunda puxada é a que dispara de fato.
...
O timeout de inatividade cai de 60 min para 5 min. **Re-comente antes de entregar para produção.**

### Lockout de bateria (só PRO)

A variante PRO monitora a tensão da bateria e protege contra descarga profunda via **modo lockout** (não corte automático — veja abaixo).

**Thresholds (por célula):**
- **Aviso de baixa**: 3,5 V/célula — beep único a cada 10 segundos
- **Crítica**: 3,2 V/célula — beep único a cada 5 segundos
- **Cut (lockout)**: 3,0 V/célula — firing bloqueado, beep a cada 5 segundos

Quando o lockout é acionado:
1. Firing fica permanentemente desabilitado até reboot (reconexão da bateria)
2. Beep de alerta toca a cada 5 segundos
3. Após ~30 minutos de beeping (360 ciclos), a FCU entra em deep sleep permanente
4. **A bateria não é cortada** — você precisa desconectar fisicamente para evitar descarga adicional

> ⚠️ **A variante PRO V2.1 NÃO tem corte automático de bateria.** Versões anteriores (V2.0 e antes) tinham um circuito de kill latch que desconectava fisicamente a bateria — isso foi removido na V2.1 para simplificar o circuito e eliminar um problema de confiabilidade no bootstrap. O modo lockout + alarme sonoro é o novo mecanismo de proteção, mas depende do usuário desconectar a bateria.
...
| Deep sleep 60 min | ✅ | ✅ |
| Alarme de inatividade (6 ciclos → sleep permanente) | ✅ | ✅ |
| **Leitura de bateria** | ❌ | ✅ |
| **Modo lockout** (bloqueio de firing em tensão crítica) | ❌ | ✅ |
| **Kill latch** (protege LiPo de descarga profunda) | ❌ | ❌ (V2.1: lockout) |
| **Botão WiFi dedicado** | ❌ | ✅ |
| **Buzzer onboard** | ❌ | ✅ |
...
3. **Sem corte automático de bateria.** A variante PRO entra em modo lockout (firing desabilitado, alarme contínuo) na tensão crítica, mas não corta fisicamente a bateria — o usuário precisa desconectar manualmente. A variante STR não tem leitura de bateria. Use com cuidado em packs 2S/3S sem proteção externa.
4. **Bateria drena enquanto está plugada.** Os reguladores de tensão consomem ~12-18mA continuamente. Um pack 1000mAh descarrega totalmente em 2-3 dias se ficar conectado. Desconecte entre sessões.
5. **Primeira puxada após deep sleep acorda via reboot.** Após 60 min de inatividade, a FCU entra em deep-sleep. A próxima puxada acorda o MCU por reboot completo — a **segunda** puxada é a que dispara. Diferente de FCUs que dormem por gate-hold do MOSFET.
...
| Deep sleep | ✅ 60 min + alarme | — | ✅ | ✅ | — |
| Corte de bateria | ❌ (PRO: lockout) | — | — | — | — |
